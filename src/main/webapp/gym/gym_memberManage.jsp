			...
			<div
				...
				<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-6">
					...
					<div class="flex flex-col gap-2.5">
						...
						<div class="flex gap-1.5">
							<a
								href="${pageContext.request.contextPath}/gym/memberManage?type=${type}&keyword=${keyword}"
								class="px-3.5 py-1.5 ${empty status ? 'bg-primary text-white' : 'bg-surface-container text-on-surface-variant'} rounded-full text-[12px] font-bold hover:shadow-md transition-all">
								전체 </a> <a
								href="${pageContext.request.contextPath}/gym/memberManage?type=${type}&status=active&keyword=${keyword}"
								class="px-3.5 py-1.5 ${status == 'active' ? 'bg-primary text-white' : 'bg-surface-container text-on-surface-variant'} hover:bg-outline-variant/30 rounded-full text-[12px] font-semibold transition-all">
								활성 </a> <a
								href="${pageContext.request.contextPath}/gym/memberManage?type=${type}&status=soon&keyword=${keyword}"
								class="px-3.5 py-1.5 ${status == 'soon' ? 'bg-primary text-white' : 'bg-surface-container text-on-surface-variant'} hover:bg-outline-variant/30 rounded-full text-[12px] font-semibold transition-all">
								만료 임박 </a> <a
								href="${pageContext.request.contextPath}/gym/memberManage?type=${type}&status=expired&keyword=${keyword}"
								class="px-3.5 py-1.5 ${status == 'expired' ? 'bg-primary text-white' : 'bg-surface-container text-on-surface-variant'} hover:bg-outline-variant/30 rounded-full text-[12px] font-semibold transition-all">
								만료 </a>
						</div>
					</div>
				</div>
				...
				<div
					...
					<form action="${pageContext.request.contextPath}/gym/memberManage"
						...
						<input type="hidden" name="type" value="${type}"> <input
							...
							placeholder="회원명 또는 전화번호 검색" type="text">
						<button type="submit"
					...
					<table class="w-full text-left">
						<thead>
							<tr
								...
								<th class="px-6 py-3 w-[12%]">이용권 현황</th>
							</tr>
						</thead>
						<tbody class="divide-y divide-outline-variant/5">
							<c:forEach var="member" items="${memberList}">
								<tr
									...
									<td class="px-6 py-3 text-center"><c:choose>
											<c:when test="${member.membershipType == 'pt'}">
												<span
													class="text-sm font-black ${member.remainPtCount <= 2 ? 'text-error' : 'text-on-surface'}">
													${member.remainPtCount}회 </span>
											</c:when>
											<c:otherwise>
									...
									<td class="px-6 py-3"><c:choose>
											<c:when test="${member.status == 'active'}">
												<div class="flex items-center gap-2">
													<span class="w-2 h-2 rounded-full bg-primary"></span> <span
														class="text-xs font-bold text-primary">활성</span>
												</div>
											</c:when>

											<c:when test="${member.status == 'soon'}">
												<div class="flex items-center gap-2">
													<span class="w-2 h-2 rounded-full bg-error"></span> <span
														class="text-xs font-bold text-error">만료 임박</span>
												</div>
											</c:when>

											<c:otherwise>
												<div class="flex items-center gap-2">
													<span class="w-2 h-2 rounded-full bg-surface-dim"></span> <span
														class="text-xs font-bold text-on-surface-variant/60">만료</span>
												</div>
											</c:otherwise>
										</c:choose></td>
								</tr>
