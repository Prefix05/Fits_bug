...
/*
 * ✅ 에러 수정 1:
 *    기존: UserDTO user = (UserDTO) request.getAttribute("member");
 *          MemberDTO member = (MemberDTO) request.getAttribute("member");
 *    → 같은 attribute를 UserDTO / MemberDTO 로 동시에 받으면 컴파일 에러
 *
 *    수정: loginUser 는 session에서, member 는 request attribute에서 각각 분리
 */
...
// MemberController에서 request.setAttribute("member", memberDTO) 로 전달됨
MemberDTO member = (MemberDTO) request.getAttribute("member");
// null 이면 세션 캐시에서 보완
if (member == null) {
    member = (MemberDTO) session.getAttribute("memberInfo");
}
// 최종 null 방어
if (member == null) {
...
// 편의 변수 (이전 코드 user.getNickname() 등 그대로 호환)
  ...
  <div style="background:white;border-radius:20px;border:1.5px solid #E8EDF5;box-shadow:0 2px 8px rgba(0,0,0,0.05);padding:26px;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
      ...
      <div style="background:#FFF3EE;border-radius:12px;padding:14px;">
        ...
        <div class="info-value"><%= member.getGoals() != null ? member.getGoals() : "미설정" %></div>
      </div>
      ...
      <div style="background:#E8F8F6;border-radius:12px;padding:14px;">
        ...
        <div class="info-value"><%= member.getExperience() != null ? member.getExperience() : "미설정" %></div>
      </div>
      <div style="background:#F7F9FC;border-radius:12px;padding:14px;">
        <div class="info-label">키 / 몸무게</div>
        <div class="info-value">
          <%= member.getHeight() > 0 ? member.getHeight() + "cm" : "-" %> /
          <%= member.getWeight() > 0 ? member.getWeight() + "kg" : "-" %>
        </div>
      </div>
      <div style="background:#FFF9E6;border-radius:12px;padding:14px;">
        <div class="info-label">식단 유형</div>
        <div class="info-value"><%= member.getDiet() != null ? member.getDiet() : "-" %></div>
      </div>
    </div>
  </div>
...
<div id="planModal" class="fb-modal-wrap">
  <div class="fb-modal-box">
    ...
    <div style="display:flex;flex-direction:column;gap:12px;">
      <!-- ✅ 모달 input도 동일하게 getGoals(), getExperience() 사용 -->
      <input id="goal"   class="fb-inp" placeholder="운동 목표 (예: 다이어트, 근육증가)" value="<%= member.getGoals() != null ? member.getGoals() : "" %>">
      <input id="level"  class="fb-inp" placeholder="운동 경험 수준 (초급/중급/고급)"    value="<%= member.getExperience() != null ? member.getExperience() : "" %>">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        <input id="height" class="fb-inp" placeholder="키 (cm)"       type="number" value="<%= member.getHeight() > 0 ? member.getHeight() : "" %>">
        <input id="weight" class="fb-inp" placeholder="몸무게 (kg)"   type="number" value="<%= member.getWeight() > 0 ? member.getWeight() : "" %>">
      </div>
      <input id="diet" class="fb-inp" placeholder="식단 유형 (예: 고단백, 저탄수)" value="<%= member.getDiet() != null ? member.getDiet() : "" %>">
    </div>
...
function render(type, data) {
  ...
  const historyEl = document.getElementById("workoutHistory");
  chartEl.style.display = foodEl.style.display = historyEl.style.display = "none";
  if (type==="workout")     { chartEl.style.display=historyEl.style.display="block"; drawWorkoutChart(data); drawWorkoutHistory(data); }
  else if (type==="food")   { foodEl.style.display="block"; currentFoodData=data; drawCalendar(); }
  else if (type==="inbody") { chartEl.style.display="block"; drawInbodyChart(data); }
}
...
function drawWorkoutChart(data) {
  ...
  const grouped = {};
  data.forEach(d => { if (!grouped[d.name]) grouped[d.name]=[]; grouped[d.name].push({date:d.date,value:(d.weight*(1+0.033*d.reps)).toFixed(1)}); });
  const colors = ["#FF6B35","#00BFA5","#9333EA","#FFD166","#FF4D4D"];
  const datasets = Object.keys(grouped).map((name,i) => ({
    label:name, data:labels.map(date=>{const f=grouped[name].find(d=>d.date===date);return f?f.value:null;}),
  ...
  chartInstance = new Chart(document.getElementById("chart"),{
    ...
    options:{plugins:{legend:{labels:{color:"#5A6480",font:{family:"'Noto Sans KR'",weight:"700"}}}},
...
function drawCalendar() {
  ...
  let html=days.map(d=>'<div style="text-align:center;font-size:12px;font-weight:700;color:#9DA8C0;padding:6px 0;">'+d+'</div>').join("");
  for(let i=1;i<=lastDate;i++){
    const full=year+"-"+String(month+1).padStart(2,"0")+"-"+String(i).padStart(2,"0");
    const d=map[full];
    html+='<div onclick="openFoodModal(\''+full+'\')" class="cal-cell'+(d?" has-data":"")+'">'+
...
function loadPaymentData() {
  const active={orderId:"ORD1234",productName:"PT 20회권 VIP",remain:20,total:20,status:"DONE",startDate:"2026-04-01",endDate:"2026-05-31"};
  const history=[{date:"2026-04-01",name:"PT 20회권 VIP",trainer:"김태훈",price:600000},{date:"2026-03-01",name:"헬스 1개월",trainer:"",price:80000}];
...
function drawMembership(data){
  ...
  let actionBtn="",overlay="",titleStyle="";
  switch(data.status){
    case "READY": actionBtn='<button onclick="cancelPayment(\''+data.orderId+'\')">❌ 결제 취소</button>'; break;
    case "DONE": case "USING":
      if(data.remain===data.total) actionBtn='<button onclick="refundPayment(\''+data.orderId+'\')">💸 100% 환불</button>';
      else if(data.remain>0) actionBtn='<button onclick="refundPayment(\''+data.orderId+'\')">💸 부분 환불</button>';
      else actionBtn='<button disabled>환불 불가</button>';
      break;
    case "CANCEL_REQ": case "REFUND_REQ":
      overlay='<div style="position:absolute;top:10px;right:10px;color:#FF6B35;font-weight:900;">처리 진행중</div>';
      titleStyle="text-decoration:line-through;"; break;
    case "CANCEL_DONE":
  ...
  box.innerHTML='<div style="position:relative;background:linear-gradient(135deg,#FF6B35,#FF8C5A);border-radius:20px;padding:28px;">'+
    overlay+'<div style="'+titleStyle+'">'+
    '<div style="color:white;">이용 중인 회원권</div>'+
    '<div style="font-size:22px;font-weight:900;color:white;">🎫 '+data.productName+'</div></div>'+
    ...
    '<div style="width:'+percent+'%;height:100%;background:white;border-radius:99px;"></div></div>'+
    '<div style="margin-top:12px;">'+actionBtn+'</div></div>';
}

function drawPaymentTable(list){
  document.getElementById("paymentTable").innerHTML=list.map(item=>
    '<tr><td>'+item.date+'</td>'+
    '<td><span style="background:#FFF3EE;color:#FF6B35;padding:4px 10px;border-radius:99px;">'+item.name+'</span></td>'+
    '<td>'+(item.trainer||"-")+'</td>'+
    '<td>'+item.price.toLocaleString()+'원</td></tr>'
  ).join("");
}

function cancelPayment(orderId){
  if(!confirm("정말 취소하시겠습니까?"))return;
  fetch("paymentAction",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:"orderId="+orderId+"&action=cancel"})
  .then(res=>res.text()).then(()=>location.reload());
}

function refundPayment(orderId){
  if(!confirm("환불 요청하시겠습니까?"))return;
  fetch("refund",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:"orderId="+orderId+"&paymentKey=PAYMENT_KEY&amount=10000"})
  .then(res=>res.json())
  .then(data=>{if(data.code)alert("환불 실패: "+data.message);else alert("환불 성공");});
}
...
function loadCommunityData(){
  const posts=[
    {id:1,title:"가슴 운동 루틴 공유 💪",content:"벤치프레스 + 인클라인 추천!",likeCount:12,commentCount:4,category:"owun"},
    {id:2,title:"다이어트 식단 기록 🥗",content:"닭가슴살 + 고구마 + 샐러드",likeCount:8,commentCount:2,category:"free"},
    {id:3,title:"하체 운동 인증 🔥",content:"스쿼트 120kg 성공!",likeCount:20,commentCount:7,category:"owun"}
  ];
  drawMyPosts(posts);
}
...
function drawMyPosts(posts){
  ...
  box.innerHTML=posts.map(post=>
    '<div onclick="goDetail('+post.id+')" class="post-mini">'+
