...
@keyframes fb_pop{0%{opacity:0;transform:scale(0.7);}60%{transform:scale(1.08);}100%{opacity:1;transform:scale(1);}}
@keyframes fb_float{0%,100%{transform:translateY(0);}50%{transform:translateY(-10px);}}
@keyframes confetti_fall{0%{opacity:1;transform:translateY(-20px) rotate(0deg);}100%{opacity:0;transform:translateY(300px) rotate(720deg);}}
</style>
</head>
<body>
...
<div style="background:white;border-radius:32px;padding:56px 48px;width:100%;max-width:460px;text-align:center;box-shadow:0 16px 60px rgba(0,0,0,0.12);position:relative;overflow:hidden;">
  ...
  <div style="position:relative;z-index:1;animation:fb_float 3s ease-in-out infinite;margin-bottom:24px;">
    <svg width="130" height="130" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" style="filter:drop-shadow(0 8px 20px rgba(0,191,165,0.3));">
      ...
      <circle cx="76" cy="60" r="15" fill="#7A7EA8"/><circle cx="124" cy="60" r="15" fill="#7A7EA8"/>
  ...
  <div style="position:relative;z-index:1;width:64px;height:64px;border-radius:50%;background:linear-gradient(135deg,#00BFA5,#26D4BB);display:flex;align-items:center;justify-content:center;margin:0 auto 20px;animation:fb_pop 0.6s ease both;box-shadow:0 6px 20px rgba(0,191,165,0.35);">
    <svg width="30" height="30" viewBox="0 0 24 24" fill="none"><path d="M5 13l4 4L19 7" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
  </div>

  <h1 style="font-size:28px;font-weight:900;color:#1A1F36;margin-bottom:10px;position:relative;z-index:1;">결제 완료! 🎉</h1>
  ...
  <div style="background:#F7F9FC;border-radius:16px;padding:20px;margin-bottom:28px;position:relative;z-index:1;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;">
      ...
      <span style="font-size:13px;font-weight:700;color:#00897B;background:#E8F8F6;padding:4px 12px;border-radius:99px;">✔ 결제 완료</span>
    </div>
  </div>
...
// 컨페티 효과
(function(){
  const colors=['#FF6B35','#FFD166','#00BFA5','#FF8C5A','#26D4BB'];
  for(let i=0;i<20;i++){
